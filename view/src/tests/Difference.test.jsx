...
describe('a call stack with a macro-function difference', () => {
  const macroFunctionSetup = () => {
    // Leaving definitions empty for simplification.
    render(
      <Difference
        compare={MacroFunctionDifference.function}
        diff={MacroFunctionDifference.diffs[0]}
        definitions={{}}
        getFile={getFile}
        oldFolder={oldFolder}
        newFolder={newFolder}
  ...
  test('both last calls in the callstack should be highlighted by default', async () => {
    macroFunctionSetup();
    const callstack = within(screen.getByTestId('callstack'));
  ...
  test('both last calls should be highlighted after selecting one of them', async () => {
    ...
    // Testing that it was selected.
    expect(callstack.getByText('down_write')).toHaveClass('active');

    // Now selecting last call in the old call stack and checking that both are selected.
    ...
    userEvent.click(callstack.getByText('down_write'));
    expect(callstack.getByText('down_write')).toHaveClass('active');
    // Selecting call in the new call stack.
